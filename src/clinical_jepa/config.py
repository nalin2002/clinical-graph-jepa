"""Configuration dataclasses for the clinical_jepa pipeline.

Merged from ``fawkes_core/config.py`` (the v4 base), ``graph_jepa_v5/config.py``
(candidate-ranking fields), and ``graph_jepa_v6/config.py`` (note-feature
fields).  The v5 and v6 ``TrainConfig`` bodies were byte-identical; the only
substantive difference between the two released variants is the four
``ModelConfig`` note fields below.

:meth:`Config.from_dict` is v6's implementation kept verbatim.  Its
backward-compatibility defaults — ``use_note_embeddings`` defaulting to
``False`` and ``base_in_dim`` being derived when absent — are precisely what
lets this one class load both released checkpoints.
"""
...
class ModelConfig:
    ...
    # Localized note features. ``in_dim == base_in_dim + note_embedding_dim``
    # when notes are used; ``in_dim == base_in_dim`` otherwise.
    base_in_dim: int = 256
    use_note_embeddings: bool = True
    note_embedding_dim: int = 768
    note_ground_by: str = "prov"  # "prov" | "name" | "all"
...
class TrainConfig:
    ...
    Staged training: the masked/JEPA objective alone, then joint fine-tuning
    with graph-revision edge plausibility and candidate ranking.  The ranking
    task hides real schema-valid edges and asks the edge head to score each
    hidden true edge above hard, schema-valid distractors from the same patient
    graph.
    """
...
class Config:
    ...
    def from_dict(cls, d: Dict[str, Any]) -> "Config":
        ...
        if "use_note_embeddings" not in model_dict:
            model_dict["use_note_embeddings"] = False
        if "note_embedding_dim" not in model_dict:
            model_dict["note_embedding_dim"] = 768
        if "note_ground_by" not in model_dict:
            model_dict["note_ground_by"] = "prov"
        if "base_in_dim" not in model_dict:
            in_dim = int(model_dict.get("in_dim", ModelConfig().in_dim))
            note_dim = int(model_dict.get("note_embedding_dim", 0))
            model_dict["base_in_dim"] = (
                max(0, in_dim - note_dim)
                if model_dict.get("use_note_embeddings")
                else in_dim
            )
