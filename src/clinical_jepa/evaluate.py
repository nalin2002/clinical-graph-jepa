"""Leave-one-out edge-recovery evaluation.

One evaluator for both released variants, merged from
``graph_jepa_v5/evaluate.py`` and ``graph_jepa_v6/evaluate.py``. Those two files
were the same 347 lines with ``s/v5/v6/``; their only behavioural difference was
that v6 passed the note arguments to ``PatientGraphDataset``. That now comes off
``cfg.model``, so the variant is a property of the checkpoint rather than of the
module you imported.

Each trusted edge is hidden one at a time and its true target ranked against the
schema-admissible candidates in the same patient graph. Nothing here is
stochastic, so a run is exactly reproducible — see ``baseline/README.md``.
"""
...
from .schema import IDX_TO_EDGE_TYPE
from .train.loop import add_data_args, build_graphs, load_model_checkpoint
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Evaluate a clinical_jepa checkpoint with leave-one-out edge recovery"
        )
    )
    ...
    parser.add_argument(
        ...
        default="schema",
        help="schema matches the ranking fine-tuning objective; "
             "same-type mirrors the v12 baseline script",
    )
    parser.add_argument(
        "--split",
        default=None,
