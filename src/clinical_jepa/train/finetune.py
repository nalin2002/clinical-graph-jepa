"""Fine-tune candidate ranking from a masked-pretrained checkpoint.

Merged from ``graph_jepa_v{5,6}/finetune.py``, which differed only in their
banner text and the two hardcoded checkpoint names. Nothing here selects a
variant: the note settings come from the pretrained checkpoint's config, so
fine-tuning cannot change what the model was pretrained as.
"""
...
from ..graph.builders import canonical_relation
from ..schema import EDGE_TYPE_TO_IDX
...
def finetune(args) -> Path:
    ...
    checkpoint_name = checkpoint_filename(cfg, pretrain=False)
    encoder = build_checkpoint_encoder(cfg, args.encoder_cache)
    dataset, train_loader = build_train_loader(args, cfg, encoder)
    print(
        f"Loaded {len(dataset)} graphs for candidate-ranking fine-tuning "
    ...
    wandb_run = init_wandb(
        ...
        cfg,
        len(dataset),
        script="clinical_jepa.train.finetune",
