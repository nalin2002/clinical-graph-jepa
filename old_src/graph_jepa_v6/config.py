"""Configuration dataclasses for Graph-JEPA v6."""
...
from fawkes_core.config import (
    ModelConfig as V4ModelConfig,
    ScoreConfig,
...
@dataclass
class ModelConfig(V4ModelConfig):
    """Architecture hyperparameters plus v6 note-feature settings."""
...
class TrainConfig(V4TrainConfig):
    ...
    v6 keeps v5 candidate-ranking fine-tuning.  A subset of
...
class Config:
    ...
    def from_dict(cls, d: Dict[str, Any]) -> "Config":
        ...
        if "base_in_dim" not in model_dict:
            in_dim = int(model_dict.get("in_dim", V4ModelConfig().in_dim))
